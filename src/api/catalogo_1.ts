...
import {
  ejecutarCargaPartidos,
  horariosCargaPartidos,
  obtenerConfiguracionCargaPartidos,
  obtenerEstadoCargaPartidosAutomatica,
...
export function registrarRutasCatalogo(
  ...
  app.get('/admin/deportes', {
    schema: {
      ...
      querystring: {
        ...
        properties: {
          ...
          soloSeleccionadas: { type: 'boolean' },
    ...
    const q = z
      .object({
        ...
        soloSeleccionadas: z.coerce.boolean().default(false),
    ...
    const [deportes, ligas, conteo, paises] = await Promise.all([
        ...
        `SELECT d.id, d.clave, d.nombre, d.tiene_empate, d.cuenta_prorroga,
                ...
                (SELECT count(*) FROM v_ligas l
                  ...
                  WHERE l.deporte_id = d.id
                    AND bl.seleccionada_panel = TRUE)::int AS activas
           FROM v_deportes d ORDER BY d.nombre`,
      ),
      pool.query(
        `SELECT l.id, l.api_id, l.nombre, l.pais, l.deporte_id,
                bl.logo_url,
                COALESCE(bl.seleccionada_panel, FALSE) AS seleccionada,
          ...
          WHERE ($1::text IS NULL OR l.pais = $1)
            ...
            AND ($5::boolean IS NOT TRUE OR bl.seleccionada_panel = TRUE)
          ORDER BY bl.seleccionada_panel DESC,
                   l.relevancia DESC,
                   l.nombre
          LIMIT $6 OFFSET $7`,
        [q.pais ?? null, q.buscar ?? null, q.soloActivas, q.deporte ?? null,
         q.soloSeleccionadas, q.limite, q.desde],
      ),
      pool.query(
        `SELECT count(*)::int AS total,
                count(*) FILTER (WHERE bl.seleccionada_panel = TRUE)::int AS seleccionadas,
                count(*) FILTER (WHERE EXISTS (
                  SELECT 1 FROM mercados_por_liga m
                   WHERE m.liga_id = l.id AND m.eliminado_en IS NULL))::int AS con_mercados
           FROM v_ligas l
           JOIN ligas bl ON bl.id = l.id
           JOIN v_deportes d ON d.id = l.deporte_id
          WHERE ($1::text IS NULL OR l.pais = $1)
      ...
      pool.query(
        `SELECT l.pais, count(*)::int AS ligas,
                count(*) FILTER (WHERE bl.seleccionada_panel = TRUE)::int AS activas
           FROM v_ligas l
           JOIN ligas bl ON bl.id = l.id
          GROUP BY l.pais ORDER BY 3 DESC, 2 DESC, 1`,
      ),
    ]);
    ...
    return {
      ...
      ligas: ligas.rows,
      paises: paises.rows,
      total: q.soloSeleccionadas ? conteo.rows[0].seleccionadas : conteo.rows[0].total,
      catalogoTotal: conteo.rows[0].total,
      seleccionadas: conteo.rows[0].seleccionadas,
      activas: conteo.rows[0].seleccionadas, // compatibilidad con el panel anterior
      conMercados: conteo.rows[0].con_mercados,
      desde: q.desde,
      limite: q.limite,
  ...
  app.put('/admin/ligas/:id/seleccion', {
    schema: {
      ...doc('Activar o desactivar una liga del catálogo', 'deportes.gestionar',
        'La selección controla qué ligas participan en la sincronización automática de partidos.'),
      params: { type: 'object', properties: { id: { type: 'string', format: 'uuid' } }, required: ['id'] },
      body: {
        type: 'object',
        properties: { seleccionada: { type: 'boolean' } },
    ...
    const { seleccionada } = z.object({ seleccionada: z.boolean() }).parse(peticion.body);

    let mercadosAuto = 0;
    await enTransaccion(async (c) => {
      const liga = await c.query(
        `SELECT l.id, d.clave AS deporte
           FROM v_ligas l
           JOIN v_deportes d ON d.id = l.deporte_id
          WHERE l.id = $1`,
        [id],
      ...
      await c.query(`UPDATE ligas SET seleccionada_panel = $2 WHERE id = $1`, [id, seleccionada]);

      // Solo en desarrollo: al activar una liga nueva se preparan los mercados
      // que el sistema ya conoce. En producción se mantienen bajo control manual.
      const autoMercados = seleccionada
        && process.env.NODE_ENV !== 'production'
        && process.env.DEV_AUTO_MERCADOS === 'true';

      if (autoMercados) {
        const existentes = await c.query(
          `SELECT count(*)::int AS n FROM mercados_por_liga
            WHERE liga_id = $1 AND eliminado_en IS NULL`,
          [id],
        );
        if (existentes.rows[0].n === 0) {
          const disponibles = MERCADOS_POR_DEPORTE[liga.rows[0].deporte] ?? [];
          ...
          mercadosAuto = disponibles.length;
        }
      }
    }, sesion.usuarioId);

    return { ok: true, seleccionada, mercadosAuto };
  });
  ...
  // ===================================================================
  //  Procesos automáticos de Deportes
  // ===================================================================

  const bodyProcesoAutomatico = z.object({
    activo: z.boolean(),
  ...
  app.get('/admin/deportes/procesos-automaticos/partidos', {
    schema: doc(
      'Configuración automática de partidos',
      'deportes.ver',
      'Devuelve ON/OFF, hora inicial, veces por día y horarios calculados.',
    ),
  }, async (peticion) => {
    await conPermiso(peticion, 'deportes.ver');
    const configuracion = await obtenerConfiguracionCargaPartidos();
    return {
      configuracion: {
        ...configuracion,
        horarios: horariosCargaPartidos(
          configuracion.hora,
          configuracion.vecesPorDia,
        ),
      },
      estado: obtenerEstadoCargaPartidosAutomatica(),
    };
  });

  app.put('/admin/deportes/procesos-automaticos/partidos', {
    schema: {
      ...doc(
        'Guardar automatización de partidos',
        'deportes.gestionar',
        'Los cambios se guardan en PostgreSQL y el scheduler los toma sin reiniciar el servidor.',
      ),
      body: {
        type: 'object',
        properties: {
    ...
    const sesion = await conPermiso(peticion, 'deportes.gestionar');
    const d = bodyProcesoAutomatico.parse(peticion.body);

    await enTransaccion(async (c) => {
      const valores: Array<[string, string]> = [
        ['deportes_auto_activo', String(d.activo)],
        ['deportes_auto_hora', d.hora],
        ['deportes_auto_veces', String(d.vecesPorDia)],
      ];
        ...
        if (actualizado.rowCount === 0) {
          ...
          await c.query(
            ...
            [
              ...
              valor,
              tipo,
              'Configuración de procesos automáticos de Deportes',
            ],
          );
        }
      }
  ...
  app.post('/admin/deportes/procesos-automaticos/partidos/ejecutar', {
    schema: doc(
      'Ejecutar carga de partidos ahora',
      'deportes.gestionar',
      'La ejecución manual funciona aunque la carga automática esté desactivada.',
    ),
  }, async (peticion) => {
    const sesion = await conPermiso(peticion, 'deportes.gestionar');
  ...
  app.post('/admin/deportes/sincronizar', {
    ...
    const activas = await pool.query(
      `SELECT count(*)::int AS n
         FROM v_ligas l JOIN ligas bl ON bl.id = l.id
    ...
    if (activas.rows[0].n === 0) {
      throw Object.assign(new Error('sin ligas'), {
        codigo: 'ENTRADA_INVALIDA',
        mensajeUsuario:
          'Ninguna liga está seleccionada. Activa al menos una desde Catálogo de ligas.',
      });
    }
  ...
  app.get('/admin/deportes/estado-proveedor', {
    ...
    const activas = await pool.query(
      `SELECT l.id, l.api_id, l.nombre, l.pais, bl.logo_url,
              l.tiene_estadisticas,
              COALESCE((SELECT count(*)::int FROM mercados_por_liga m
                         WHERE m.liga_id = l.id AND m.eliminado_en IS NULL), 0) AS mercados,
              COALESCE((SELECT count(*)::int FROM v_partidos p
                         WHERE p.liga_id = l.id AND p.inicia_en > now()), 0) AS partidos
         FROM v_ligas l
         JOIN ligas bl ON bl.id = l.id
        WHERE bl.seleccionada_panel = TRUE
        ORDER BY l.relevancia DESC, l.nombre`,
    );
